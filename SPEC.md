...
- `ServiceTier` — `fast`. Resolved from an exact, case-insensitive `fast` or `priority` token in a recognized Codex model's `model.display_name`. Drives the `🚀` badge.
...
- **REQ-SL-099**: Codex Fast service-tier badge. A recognized Codex model whose `model.display_name` contains the case-insensitive exact token `fast` or `priority` renders `🚀` directly after the reasoning-effort badge, or directly after the model glyph when effort is absent. The two labels represent the same Fast mode; served OpenAI responses normalize either request alias to `priority`. `ultrafast`, default, absent, unknown, substring matches, and non-Codex models hide the badge. Resolution is payload-only and adds no subprocesses or file reads. Source: [OpenAI Responses API `service_tier`](https://developers.openai.com/api/reference/cli/resources/responses/methods/create).
...
- [x] `test/codex.json` carries the served `priority` suffix for replay smoke coverage.
