...
- A configured window also decides which usage source wins. A producer's `used_percentage` is measured against that producer's own full context window, so it is authoritative only while no window is configured; once one is, exact `current_usage` token counts lead and a percentage-only producer is re-based onto the effective ceiling. Trusting the producer's figure verbatim is what made a resolved ceiling render as if it were absent.
