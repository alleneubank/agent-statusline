...
- *Provisional 2026-08-28:* a typed mission root may add one `missionctl statusline` subprocess. Two root-file existence probes keep untyped repositories on the prior process budget; the subprocess output is capped, fully drained, and fail-open. This is the canonical-projection exception to the Latency dimension, parallel to the existing delegated `rl statusline` segment.
