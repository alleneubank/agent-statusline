...
- Claude Code and Codex-style status payloads are supported. Claude models render with their existing glyphs; the GPT-5.6 family renders with model-specific glyphs (`gpt-5.6-sol` ☀️, `gpt-5.6-terra` 🌍, `gpt-5.6-luna` 🌙). Previous-generation Codex glyphs remain supported, with `⌘` as the generic GPT/Codex fallback. Kimi models (`kimi-code/*`, `k3[1m]`, `Kimi K2`) render as 🌑.
